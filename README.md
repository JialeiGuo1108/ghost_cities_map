...
The design employs a dark-themed interface with a three-panel layout, where the left and right sidebars contain statistical information and comparative charts, while the central panel features an interactive global map. The color scheme uses high-contrast gold/amber highlights against a black background, with a carefully designed color progression to represent the Ghost City Index severity. The visualization contains multiple linked views to allow users to understand the phenomenon from different perspectives:
